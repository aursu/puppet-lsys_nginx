...
class lsys_nginx (
  ...
  Optional[Stdlib::Unixpath] $log_directory = undef,
  ...
  Integer $daemon_group_id                  = $bsys::webserver::params::group_id,
  Stdlib::Unixpath $web_server_user_shell   = $bsys::webserver::params::user_shell,
  Stdlib::Unixpath $cache_directory         = $lsys_nginx::params::cachedir,
  Optional[Stdlib::Unixpath] $lib_directory = $lsys_nginx::params::libdir,
  Stdlib::Unixpath $proxy_temp_path         = $lsys_nginx::params::proxy_temp_path,
  Stdlib::Unixpath $user_home               = $lsys_nginx::params::user_home,
  Stdlib::Unixpath $map_dir                 = $lsys_nginx::params::map_dir,
) inherits lsys_nginx::params {
  $defined_conf_dir = $conf_dir ? {
    Stdlib::Unixpath => $conf_dir,
    default          => $nginx::params::conf_dir,
  }

  $defined_package_name = $package_name ? {
    String  => $package_name,
    default => $nginx::params::package_name,
  }

  $defined_pid_path = $pid_path ? {
  ...
  if $manage_user {
    ...
    if $manage_user_home and !($user_home in $runtime_directories) {
      file { $user_home:
        ensure => directory,
        owner  => $resource_owner,
  ...
  if $log_directory {
    ...
    unless $log_directory == $nginx::params::log_dir {
      file { $log_directory:
        ensure  => directory,
        group   => $daemon_group,
        owner   => $daemon_user,
    ...
    if $facts['os']['selinux']['enabled'] {
      selinux::fcontext { "${log_directory}(/.*)?":
        filetype => 'f',
        seltype  => 'httpd_log_t',
      ...
      selinux::exec_restorecon { $log_directory:
        subscribe => Selinux::Fcontext[$log_directory],
      }
    }
  }
  ...
  class { 'nginx':
    ...
    package_name             => $defined_package_name,
    package_source           => $package_source,
    pid                      => $defined_pid_path,
